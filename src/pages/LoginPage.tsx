...
type Step = 'form' | 'otp-login' | 'otp-verify';
...
export default function LoginPage() {
  ...
  const [otpPurpose, setOtpPurpose] = useState<'verify' | 'login'>('login');
      ...
      <main className="w-full max-w-md">
                ...
                >
                  {loading ? (
                    <span className="flex items-center justify-center gap-2">
                      ...
                      {mode === 'login' ? 'Memeriksa...' : 'Mendaftar...'}
                    </span>
                  ) : mode === 'login' ? 'Masuk → Kirim OTP' : 'Daftar → Kirim OTP'}
                </button>
              </form>

              {/* Hiding Registration Link if configuration is disabled */}
        ...
        {step === 'form' && (
          <div className="flex items-center justify-center gap-2 mt-4 text-xs text-gray-500">
            ...
            Login dilindungi dengan verifikasi OTP via email
          </div>
        )}
      </main>
    </div>
