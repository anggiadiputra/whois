...
import { Users, Plus, Search, Trash2, Edit2, Shield, Mail, User, Key, ChevronLeft, ChevronRight, X } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Kelola akun pengguna dan hak akses peran (Role).</p>
        </div>
        <button
      ...
      {error && (
        <div className="p-4 bg-red-50 border border-red-100 rounded-xl flex items-center gap-3 text-red-700 text-sm font-medium">
          ...
          <span>{error}</span>
          <button onClick={fetchUsers} className="ml-auto underline hover:text-red-900">Ulangi</button>
        </div>
      )}
                    ...
                    {paginatedData.map((usr: any, idx) => (
                      <tr key={usr.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-xs">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border ${usr.role === 'admin' ? 'bg-black text-white border-black' : 'bg-gray-50 text-gray-600 border-gray-200'}`}>
                            {usr.role}
                          </span>
                        </td>
                ...
                {paginatedData.map((usr: any) => (
                  <div key={usr.id} className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${usr.role === 'admin' ? 'bg-black text-white border-black' : 'bg-gray-50 text-gray-600 border-gray-200'}`}>
                        {usr.role}
                      </span>
                    </div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                {Array.from({ length: totalPages }).map((_, idx) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-lg text-xs font-semibold flex items-center justify-center transition-all ${validPage === idx + 1 ? 'bg-black text-white' : 'border border-gray-200 text-gray-600 hover:bg-white'}`}
                  >
                    {idx + 1}
                  </button>
                ))}
